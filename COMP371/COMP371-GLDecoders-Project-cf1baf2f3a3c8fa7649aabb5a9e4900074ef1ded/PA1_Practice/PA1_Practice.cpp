...
const float surfaceThreshold = 138;
...
ISoundSource * musicArea1 = SoundEngine->addSoundSourceFromFile("../Audio/s2.mp3");
...
#pragma region Create the unit "building block" meshes that we use for other things (cubes,spheres,etc.)
	...
	// SET UP OUR HIERARCHICAL MODELS FROM A2

	// Nicholas' Model
	// The 'C'
	HierarchicalModel nicksHierarchicalModel(new Child(NULL, true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, translate(vec3(-4, 1.2, 0)) * scale(vec3(6.0f, 2.4f, 2.4f)), &Nicks_Transform, depthCubeMap), &worldMatrix, true, gold);
	nicksHierarchicalModel.setChild(new Child(nicksHierarchicalModel.base(), true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, translate(vec3(-0.375, 1.75f, 0))* scale(vec3(0.25f, 2.5f, 1.0f)), &Nicks_Transform, depthCubeMap));
	nicksHierarchicalModel.setChild(new Child(nicksHierarchicalModel.last(), true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, scale(vec3(4.0f, 1.0f / 2.5f, 1.0f)) * translate(vec3(0.375, 1.75, 0)), &Nicks_Transform, depthCubeMap));
	// Now the '3'
	nicksHierarchicalModel.setChild(new Child(nicksHierarchicalModel.base(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, translate(vec3(8.0 / 6.0, 0, 0)), &Nicks_Transform, depthCubeMap));
	nicksHierarchicalModel.setChild(new Child(nicksHierarchicalModel.last(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, translate(vec3(0.375, 1.75f, 0))* scale(vec3(0.25f, 2.5f, 1.0f)), &Nicks_Transform, depthCubeMap));
	nicksHierarchicalModel.setChild(new Child(nicksHierarchicalModel.last(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, scale(vec3(4.0f, 1.0f / 2.5f, 1.0f)) * translate(vec3(-0.375, 1.75, 0)), &Nicks_Transform, depthCubeMap));
	nicksHierarchicalModel.setChild(new Child(nicksHierarchicalModel.last(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, scale(vec3(6.0f / 8.0f, 1.0f, 1.0f)) * translate(vec3(-0.1675, -1.75, 0)), &Nicks_Transform, depthCubeMap, tileSpecMapID1));

	// Ale's Model
	// The 'E'
	HierarchicalModel alesHierarchicalModel(new Child(NULL, true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, translate(vec3(-4, 1.5f, 0)) * scale(vec3(10.0f, 3.0, 4.0f)), &Ales_Transform, depthCubeMap), &worldMatrix, true, Ruby);
	alesHierarchicalModel.setChild(new Child(alesHierarchicalModel.base(), true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, translate(vec3(-0.375, 2.5f, 0))* scale(vec3(0.25f, 4.0, 1.0f)), &Ales_Transform, depthCubeMap));
	alesHierarchicalModel.setChild(new Child(alesHierarchicalModel.base(), true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, translate(vec3(0.0f, 5.0f, 0))* scale(vec3(1.0f, 1.0f, 1.0f)), &Ales_Transform, depthCubeMap));
	alesHierarchicalModel.setChild(new Child(alesHierarchicalModel.base(), true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, translate(vec3(0.125f, 2.5f, 0))* scale(vec3(0.75f, 1.0f, 1.0f)), &Ales_Transform, depthCubeMap));
	// Now the '1'
	alesHierarchicalModel.setChild(new Child(alesHierarchicalModel.base(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, translate(vec3(1.5f, 2.5f, 0)) * scale(vec3(0.30f, 6.0f, 1.0f)), &Ales_Transform, depthCubeMap));
	alesHierarchicalModel.setChild(new Child(alesHierarchicalModel.base(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, translate(vec3(1.2f, 5.25f, 0)) * scale(vec3(0.30f, 0.50f, 1.0f)), &Ales_Transform, depthCubeMap, tileTextureID2));

	// Mohamad's Model
	// The 'H'
	HierarchicalModel mohamadsHierarchicalModel(new Child(NULL, true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, translate(vec3(-7.0, 5.4, 0)) * scale(vec3(2.4f, 10.8f, 2.4f)), &Mohamads_Transform, depthCubeMap), &worldMatrix, true, Ruby);
	mohamadsHierarchicalModel.setChild(new Child(mohamadsHierarchicalModel.base(), true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, translate(vec3(2.0, 0.0f, 0))* scale(vec3(1.0f, 1.0f, 1.0f)), &Mohamads_Transform, depthCubeMap));
	mohamadsHierarchicalModel.setChild(new Child(mohamadsHierarchicalModel.last(), true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, scale(vec3(1.0f, 2.4 / 10.8, 1.0f)) * translate(vec3(-1.0, 0.0, 0)), &Mohamads_Transform, depthCubeMap));
	// Now the '6'
	mohamadsHierarchicalModel.setChild(new Child(mohamadsHierarchicalModel.base(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, translate(vec3(3.8, 0, 0)), &Mohamads_Transform, depthCubeMap));
	mohamadsHierarchicalModel.setChild(new Child(mohamadsHierarchicalModel.last(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, translate(vec3(1, 0, 0))* scale(vec3(1.0f, 2.4 / 10.8f, 1.0f)), &Mohamads_Transform, depthCubeMap));
	mohamadsHierarchicalModel.setChild(new Child(mohamadsHierarchicalModel.last(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, translate(vec3(0, -1.8, 0))* scale(vec3(1.0f, 1.0, 1.0f)), &Mohamads_Transform, depthCubeMap));
	mohamadsHierarchicalModel.setChild(new Child(mohamadsHierarchicalModel.last(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, translate(vec3(1, 1.8 / 2 + 0.02, 0))* scale(vec3(1.0, 2.75, 1.0f)), &Mohamads_Transform, depthCubeMap));
	mohamadsHierarchicalModel.setChild(new Child(mohamadsHierarchicalModel.last(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, translate(vec3(-0.5, 0.971, 0))* scale(vec3(2.0f, 3.0 / 9.0, 1.0f)), &Mohamads_Transform, depthCubeMap, lavaTextureID1));

	// Adrien's Model
	// The 'R'
	HierarchicalModel adriensHierarchicalModel(new Child(NULL, true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, translate(vec3(-4.5, 5.625, 0)) * scale(vec3(2.25f, 2.25f, 2.25f)), &Adriens_Transform, depthCubeMap), &worldMatrix, true, Ruby);
	adriensHierarchicalModel.setChild(new Child(adriensHierarchicalModel.last(), true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, translate(vec3(0.0f, 2.0f, 0.0f)), &Adriens_Transform, depthCubeMap));
	adriensHierarchicalModel.setChild(new Child(adriensHierarchicalModel.last(), true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, translate(vec3(1.0f, -1.0f, 0.0f)), &Adriens_Transform, depthCubeMap));
	adriensHierarchicalModel.setChild(new Child(adriensHierarchicalModel.last(), true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, translate(vec3(0.0f, -2.5f, 0.0f)) * scale(vec3(1.0f, 2.0f, 1.0f)), &Adriens_Transform, depthCubeMap));
	adriensHierarchicalModel.setChild(new Child(adriensHierarchicalModel.last(), true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, translate(vec3(-2.0f, 0.75f, 0.0f)) * scale(vec3(1.0f, 2.5f, 1.0f)), &Adriens_Transform, depthCubeMap));
	// Now the '9'
	adriensHierarchicalModel.setChild(new Child(adriensHierarchicalModel.base(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, translate(vec3(3.0f, 1.0f, 0.0f)), &Adriens_Transform, depthCubeMap));
	adriensHierarchicalModel.setChild(new Child(adriensHierarchicalModel.last(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, translate(vec3(0.5f, 1.0f, 0.0f)) * scale(vec3(2.0f, 1.0f, 1.0f)), &Adriens_Transform, depthCubeMap));
	adriensHierarchicalModel.setChild(new Child(adriensHierarchicalModel.last(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, translate(vec3(0.0f, -2.0f, 0.0f)), &Adriens_Transform, depthCubeMap));
	adriensHierarchicalModel.setChild(new Child(adriensHierarchicalModel.last(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, translate(vec3(0.25f, -2.0f, 0.0f)) * scale(vec3(1.5f, 1.0f, 1.0f)), &Adriens_Transform, depthCubeMap));
	adriensHierarchicalModel.setChild(new Child(adriensHierarchicalModel.last(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, scale(vec3(1.0f / 3.0f, 1.0f, 1.0f)) * translate(vec3(1.0f, 2.5f, 0.0f)) * scale(vec3(1.0, 4.0f, 1.0f)), &Adriens_Transform, depthCubeMap, iceTextureID1));

	// Sami's Model
	// The 'M'
	HierarchicalModel samisHierarchicalModel(new Child(NULL, true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, translate(vec3(-12.0f, 7.2f, 0.0f)) * scale(vec3(2.4f, 14.4f, 2.4f)), &Samis_Transform, depthCubeMap), &worldMatrix, true, chrome);
	samisHierarchicalModel.setChild(new Child(samisHierarchicalModel.base(), true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, scale(vec3(1.0f, 1.0f / 6.0f, 1.0f)) * translate(vec3(1.0f, 2.0f, 0.0f)), &Samis_Transform, depthCubeMap));
	samisHierarchicalModel.setChild(new Child(samisHierarchicalModel.last(), true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, translate(vec3(1.0f, -1.0f, 0.0f)), &Samis_Transform, depthCubeMap));
	samisHierarchicalModel.setChild(new Child(samisHierarchicalModel.last(), true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, translate(vec3(1.0f, 1.0f, 0.0f)), &Samis_Transform, depthCubeMap));
	samisHierarchicalModel.setChild(new Child(samisHierarchicalModel.base(), true, &c, &uS, &UIM.primitiveType, woodBoxTextureID1, scale(vec3(1.0f, 1.0f, 1.0f)) * translate(vec3(4.0f, 0.0f, 0.0f)), &Samis_Transform, depthCubeMap));
	// Now the '3'
	samisHierarchicalModel.setChild(new Child(samisHierarchicalModel.base(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, translate(vec3(8.0, 0, 0)), &Samis_Transform, depthCubeMap));
	samisHierarchicalModel.setChild(new Child(samisHierarchicalModel.last(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, translate(vec3(-1.5f, -1.0f / 2.4f, 0.0f))* scale(vec3(2.0f, 1.0 / 6.0f, 1.0f)), &Samis_Transform, depthCubeMap));
	samisHierarchicalModel.setChild(new Child(samisHierarchicalModel.last(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, translate(vec3(0.0f, 2.5f, 0)), &Samis_Transform, depthCubeMap));
	samisHierarchicalModel.setChild(new Child(samisHierarchicalModel.last(), false, &c, &uS, &UIM.primitiveType, iceTextureID1, translate(vec3(0.0f, 2.5f, 0)), &Samis_Transform, depthCubeMap, blueGlassTextureID1));
...
#pragma endregion
	...
	/*
	vec3 cornerSpheres[xMag][yMag][zMag][8];
	//mat4 spherePositions[xMag][yMag][zMag][8];
	for (int x = -xMag / 2; x < xMag / 2; ++x) for (int y = -yMag / 2; y < yMag / 2; ++y) for (int z = -zMag / 2; z < zMag / 2; ++z) {
	vec3 p(x, y, z);
	for (int i = 0; i < 8; ++i) {
	cornerSpheres[x + xMag / 2][y + yMag / 2][z + zMag / 2][i] = p + corners2[i].pos;

	/*spherePositions[x + xMag / 2][y + yMag / 2][z + zMag / 2][i]
	= worldMatrix * scale(translate(u,
	cornerSpheres[x + xMag / 2][y + yMag / 2][z + zMag / 2][i]), vec3(0.2));*/
	//	}
	//}
...
#pragma endregion
		...
		//  drawCoordinateAxes(lx, lineShader, primitiveType, cameraView, cameraProjection);
		...
		// DRAW ALL OF OUR MODELS

		sphereShader.prepare(CVI, VCP, UIDV);
		
		sphereShader.setCrystalLights(curChunk.crystalLights);
		sphereShader.setFloat("NUM_CRYSTALS_IN_CHUNK", (float)curChunk.crystalLights.size());

		cubeShader.prepare(CVI, VCP, UIDV);
		// Set Crystal light uniforms
		cubeShader.setCrystalLights(curChunk.crystalLights);
		cubeShader.setFloat("NUM_CRYSTALS_IN_CHUNK", (float)curChunk.crystalLights.size());


		if (vec3(Nicks_Transform[3]) != vec3(0))  nicksHierarchicalModel.Draw(&cubeShader, &sphereShader, worldMatrix);

		if (vec3(Ales_Transform[3]) != vec3(0)) alesHierarchicalModel.Draw(&cubeShader, &sphereShader, worldMatrix);

		if (vec3(Samis_Transform[3]) != vec3(0)) 	samisHierarchicalModel.Draw(&cubeShader, &sphereShader, worldMatrix);

		if (vec3(Mohamads_Transform[3]) != vec3(0)) mohamadsHierarchicalModel.Draw(&cubeShader, &sphereShader, worldMatrix);

		if (vec3(Adriens_Transform[3]) != vec3(0)) 	adriensHierarchicalModel.Draw(&cubeShader, &sphereShader, worldMatrix);
		...
		glBindTexture(GL_TEXTURE_CUBE_MAP, depthCubeMap);
		glDrawArrays(primitiveType, 0, vc);
		*/
		...
		// GRASS SHADER TESTING
		/*

		testGrassShader.prepare(CVI, VCP, UIDV, SunLight);
		testGrassShader.setModelMatrix(worldMatrix);
		testGrassShader.setFloat("time", cFrame);
		glDrawArrays(primitiveType, 0, vc);
		*/
		...
		*/
		...
		testGrassShader.setModelMatrix(worldMatrix);
		testGrassShader.setFloat("time", cFrame);
		testGrassShader.setVec3("playerPosition", player.Position);
		testGrassShader.setVec3("playerDirection", player.Front);
		...
		glEnableVertexAttribArray(0);
		glEnableVertexAttribArray(1);
		glDrawArrays(GL_POINTS, 0, 20880);
		glDisableVertexAttribArray(1);
		glDisableVertexAttribArray(0);
		glBindVertexArray(0);
		}
	...
	//	std::cout << " __________________________" << std::endl;
		//std::cout << "Position associated for collisions: " << to_string(playerCollisionPosition) << std::endl;
